	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if isConfigLoaded == true {
			...
			if len(config.ProjectId) == 0 {
				projectId, err = getProjectIdAndUpdateProjectConfigFile(config.CompanyId, config.MemberId, config.SessionToken, env)
				if err != nil {
					return
				}
			} else {
			...
			items, err := api.FetchProjectItems(config.CompanyId, config.MemberId, projectId, config.SessionToken, env)
			if err != nil {
