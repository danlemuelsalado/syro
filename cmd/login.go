...
var loginCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(token) > 0 && len(projectId) > 0 {
			companyId, validatedAccessToken, validatedProjectId, err := api.ValidateAccessTokenAndProjectId(token, projectId, env)
			if err != nil {
		...
		if isConfigLoaded == true {
			isSessionTokenValid, err := api.ValidateSessionToken(config.SessionToken, env)
			if err != nil {
				fmt.Println("Something went wrong while validating your session token. We recommend logging in again.")
				companyId, memberId, sessionToken, err := loginAndUpdateProjectConfigFile(env)
				if err != nil {
					return
				}
				_, err = getProjectIdAndUpdateProjectConfigFile(companyId, memberId, sessionToken, env)
				if err != nil {
					return
				}
			}
			if !isSessionTokenValid {
				...
				if len(config.ProjectId) == 0 {
					_, err = getProjectIdAndUpdateProjectConfigFile(config.CompanyId, config.MemberId, config.SessionToken, env)
					if err != nil {
						return
					}
				} else {
...
func loginAndUpdateProjectConfigFile(env string) (companyId string, memberId string, sessionToken string, err error) {
	...
	companyId, expiresAt, memberId, sessionToken, err := api.Login(email, password, env)
	if err != nil {
...
func getProjectIdAndUpdateProjectConfigFile(companyId string, memberId string, sessionToken string, env string) (projectId string, err error) {
	...
	isProjectIdValid, err := api.ValidateProjectId(companyId, memberId, userProjectId, sessionToken, env)
	if !isProjectIdValid {
